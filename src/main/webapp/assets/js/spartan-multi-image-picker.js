...
(function ($) {
    ...
    $.fn.spartanMultiImagePicker = function (options) {
        ...
        function addRow(settings, el) {
            ...
            if (itemsNo < 2 ) {
                last_index = count;
                var groupClassName = settings.groupClassName, rowHeight = settings.rowHeight,
                    fieldName = settings.fieldName, placeholderImage = settings.placeholderImage,
                    dropFileLabel = settings.dropFileLabel;
                ...
                var template = `<div class="${groupClassName} spartan_item_wrapper" data-spartanindexrow="${count}" style="margin-bottom : 20px; ">` +
                    ...
                    `${uploadLoaderIcon}` +
                    `</div>` +
                    `<label class="file_upload" style="width: 350px; height: ${rowHeight}; border: 2px dashed #ddd; border-radius: 3px; cursor: pointer; text-align: center; overflow: hidden; padding: 5px; margin-top: 5px; margin-bottom : 5px; position : relative; display: flex; align-items: center; margin: auto; justify-content: center; flex-direction: column;">` +
                    ...
                    `<img style="width: 100%;height: 100%;object-fit: contain; vertical-align: middle; display:none;" class="img_" data-spartanindeximage="${count}">` +
